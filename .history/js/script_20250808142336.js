document.addEventListener('DOMContentLoaded', () => {
    ...
    rigthArrow.addEventListener('click', (e) => {
        if (offset == parseInt(width) * (sliderImgs.length - 1)) offset == 0;
        else {
            offset += parseInt(width);
        }
        slidesField.transform = `translateX(-${offset})`;
        currentIndex = Math.floor(offset / (sliderImgs.length * parseInt(width)));
        currentPage.textContent = `0${currentIndex}`;
