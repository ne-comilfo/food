document.addEventListener('DOMContentLoaded', () => {
    ...
    let checkFull = true;
    params.forEach(key => {
        if (params[key]) checkFull = false;
    })
    let resultKkal;
    if (checkFull) {
        const genderP = coeefs[params.gender];
        resultKkal = coeefs[params.activity] * (genderP.first + (genderP.weight * params.weight) + (genderP.height * params.height) - (genderP.age * params.age))
        calcResult.innerHTML = `<span>${resultKkal}</span> ккал`
    }
})